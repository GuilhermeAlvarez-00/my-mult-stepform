...
export const Input = ({ error, className, onChange, ...rest }: IInput) => {
	return (
		<article className={twMerge("w-full relative", className?.root)}>
			...
			<input
				...
				className={twMerge(`border border-gray-200 rounded-sm sm:rounded-lg w-full p-2.5 focus:outline-none focus:border-purple-600 text-blue-950 ${error && "border-red-500"}`, className?.input)}
				{...rest}
			/>
		</article>
	)
